...
#define DEADTIME_US 300
// SHA-256 に食わせるイベント数(=収集バイト数)→ 出力32B。
// 出力32Bに申告できるのは最大256bitなので、256イベントだと「1イベント=1bit」がそのまま
// 上限に張り付き、安全余裕がなくなる。512 にして 1イベントあたり 0.5bit の申告に落とす。
// キット側の時刻量子化で Δt のエントロピーが目減りしていても、これなら過大申告になりにくい。
#define WHITEN_EVENTS 512
