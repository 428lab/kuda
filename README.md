...
でログイン**してAPIキーを発行・管理できる(有効キー5本まで、既定30滴/日)。
発行された平文キーは**その画面で一度しか表示されない**。
...
認証: `Authorization: Bearer kuda_...`(APIキー)。キーは `?key=` では渡せない
(URLはログに残るため)。任意で `?client_id=<英数32文字>` または `X-Client-Id`
ヘッダを付けると、どのクライアントが引いたか監査ログに記録される。
...
### `POST /admin/keys` (要 `Authorization: Bearer <INGEST_TOKEN>`・暫定)
APIキーの発行。body: `{"label": "project-a", "daily_quota": 100}`。
**平文キーはこの応答で一度だけ返る**(保存されるのはSHA-256のみ)。
ダッシュボード実装後にNostr管理者認証へ置き換え予定。
